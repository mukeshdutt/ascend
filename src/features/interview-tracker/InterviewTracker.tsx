...
import { PRIORITIES, STATUS_LABEL, TOPIC_STATUSES, TRACKS } from './types'
import type { InterviewTrack, Topic, TopicPriority, TopicStatus } from './types'
import './interview-tracker.css'

type Form = Omit<Topic, 'id'>
const blank = (): Form => ({ track: TRACKS[0], title: '', subtopic: '', priority: 'P1', status: 'not-started', resource_link: '', notes: '', last_studied_date: '', next_revisit_date: '' })
...
export function InterviewTracker() {
  const { topics, patchTopic, bulkDone } = useInterviewTopics()
  const [view, setView] = useState<'tracks' | 'board' | 'overview'>('tracks')
  const [track, setTrack] = useState<InterviewTrack>(TRACKS[0])
  ...
  const filtered = useMemo(() => topics.filter(t => (view !== 'tracks' || t.track === track) && (priority === 'all' || t.priority === priority) && (status === 'all' || t.status === status)).sort((a,b) => priorityRank[a.priority] - priorityRank[b.priority] || (a.last_studied_date || '0000').localeCompare(b.last_studied_date || '0000')), [topics, view, track, priority, status])
  const activeTracks = new Set(topics.filter(t => t.status === 'in-progress').map(t => t.track)).size
  ...
  return <div className="content interview-tracker">
    <section className="intro"><div><h1>Interview 2026</h1><p>One focused master plan across your senior engineering transition.</p></div><button className="btn-primary" onClick={() => setModal('new')}><Icon name="plus" size={16} /> Add topic</button></section>
    ...
    {view === 'tracks' && <><div className="track-tabs">{TRACKS.map(t => <button key={t} className={track === t ? 'active' : ''} onClick={() => { setTrack(t); setSelected([]) }}>{t}</button>)}</div>{selected.length > 0 && <div className="bulk-bar"><span>{selected.length} selected</span><button onClick={() => { bulkDone(selected); setSelected([]) }}><Icon name="check" size={15} /> Mark as done</button></div>}<div className="topic-table-wrap"><table className="topic-table"><thead><tr><th></th><th>Topic</th><th>Priority</th><th>Status</th><th>Last studied</th><th>Next revisit</th><th></th></tr></thead><tbody>{filtered.map(t => <tr key={t.id}><td><input type="checkbox" checked={selected.includes(t.id)} onChange={() => toggle(t.id)} /></td><td><button className="topic-title" onClick={() => setModal(t)}>{t.title}<small>{t.subtopic}</small></button></td><td><span className={`priority ${t.priority.toLowerCase()}`}>{t.priority}</span></td><td><select className={`status-select ${t.status}`} value={t.status} onChange={e => updateStatus(t.id, e.target.value as TopicStatus)}>{TOPIC_STATUSES.map(s => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}</select></td><td>{t.last_studied_date || '—'}</td><td>{t.next_revisit_date || '—'}</td><td><button className="row-edit" onClick={() => setModal(t)} aria-label={`Edit ${t.title}`}><Icon name="edit" size={15} /></button></td></tr>)}{filtered.length === 0 && <tr><td colSpan={7} className="empty">No topics match these filters.</td></tr>}</tbody></table></div></>}
    {view === 'board' && <div className="kanban">{TOPIC_STATUSES.map(s => <section key={s} className="kanban-column" onDragOver={e => e.preventDefault()} onDrop={() => { if (dragged) updateStatus(dragged, s); setDragged(null) }}><header><span className={`status-dot ${s}`} />{STATUS_LABEL[s]} <b>{filtered.filter(t => t.status === s).length}</b></header>{filtered.filter(t => t.status === s).map(t => <article key={t.id} draggable onDragStart={() => setDragged(t.id)} className="kanban-card" onClick={() => setModal(t)}><span>{t.track}</span><strong>{t.title}</strong><em className={`priority ${t.priority.toLowerCase()}`}>{t.priority}</em></article>)}</section>)}</div>}
