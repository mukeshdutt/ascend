...
export function InterviewTracker() {
  ...
  return <div className="content interview-tracker" style={ACCENT}>
    ...
    {view === 'tracks' && <><div className="track-tabs">{TRACKS.map(t => <button key={t} className={track === t ? 'active' : ''} onClick={() => { setTrack(t); setSelected([]) }}>{t}</button>)}</div><p className="track-note">{TRACK_NOTE[track]}</p>{selected.length > 0 && <div className="bulk-bar"><span>{selected.length} selected</span><button onClick={() => { bulkDone(selected); setSelected([]) }}><Icon name="check" size={15} /> Mark as done</button></div>}<div className="topic-table-wrap"><table className="topic-table"><thead><tr><th></th><th>Topic</th><th>Priority</th><th>Status</th><th>Last studied</th><th>Next revisit</th><th></th></tr></thead><tbody>{PHASES.map(ph => { const rows = filtered.filter(t => t.phase === ph); if (rows.length === 0) return null; return <Fragment key={ph}><tr className="phase-row"><td colSpan={7}><strong>{PHASE_LABEL[ph]}</strong><em>{PHASE_DESC[ph]}</em></td></tr>{rows.map(t => <tr key={t.id}><td><input type="checkbox" checked={selected.includes(t.id)} onChange={() => toggle(t.id)} /></td><td><button className="topic-title" onClick={() => setModal(t)}>{t.title}<small>{t.subtopic}</small></button></td><td><span className={`priority ${t.priority.toLowerCase()}`}>{t.priority}</span></td><td><select className={`status-select ${t.status}`} value={t.status} onChange={e => updateStatus(t.id, e.target.value as TopicStatus)}>{TOPIC_STATUSES.map(s => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}</select></td><td>{t.last_studied_date || '—'}</td><td>{t.next_revisit_date || '—'}</td><td><button className="row-edit" onClick={() => setModal(t)} aria-label={`Edit ${t.title}`}><Icon name="edit" size={15} /></button></td></tr>)}</Fragment> })}{filtered.length === 0 && <tr><td colSpan={7} className="empty">No topics match these filters.</td></tr>}</tbody></table></div></>}
